...
/*   Updated: 2021/01/08 17:10:22 by tlemesle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int		ft_other_convert(char c, va_list ap, t_flags flags)
{	
	if (c == '%')
	{
...
int		ft_hexa_convert(char c, va_list ap, t_flags flags)
{
	...
	u_nb = 4294967295 + (int) va_arg(ap, int) + 1;
//	printf("\nu_nb = %u\n", u_nb);
	baseset.nbr_base = 0;
	baseset.base = "";
	arg = ft_u_itoa(u_nb, c, baseset);
//	printf("\n\narg = %s\n", arg);
	if (ft_is_flags_empty(flags))
...
int	ft_convert(char c, t_flags flags, va_list ap)
{
	...
	if (ft_is_other_convertor(c))
		return (ft_other_convert(c, ap, flags));
	return (0);
}